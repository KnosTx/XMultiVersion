...
use KnosTx\XMultiVersion\network\convert\XMultiVersionItemTranslator;
...
class Serializer{
    ...
    public static function putItemStack(NetworkBinaryStream $packet, int $protocol, Item $item, callable $writeExtraCrapInTheMiddle) {
        ...
        [$netId, $netData] = XMultiVersionItemTranslator::getInstance()->toNetworkId($item->getId(), $coreData, $protocol);
        ...
        if($isBlockItem){
            ...
            if($block->getId() !== BlockTypeIds::AIR){
                $blockRuntimeId = XMultiVersionRuntimeBlockMapping::toStaticRuntimeId($block->getId(), $block->getDamage(), $protocol);
            }
        }
        ...
        $packet->putString(
            (static function() use ($protocol, $nbt, $netId) : string{
                ...
                if($netId === XMultiVersionItemTypeDictionary::getInstance()->fromStringId("minecraft:shield", $protocol)){
    ...
    public static function putRecipeIngredient(NetworkBinaryStream $packet, Item $item, int $protocol) {
        if($item->isNull()){
            ...
            if($item->hasAnyDamageValue()){
                [$netId, ] = XMultiVersionItemTranslator::getInstance()->toNetworkId($item->getId(), 0, $protocol);
                $netData = 0x7fff;
            }else{
                [$netId, $netData] = XMultiVersionItemTranslator::getInstance()->toNetworkId($item->getId(), $item->getDamage(), $protocol);
            }
            $packet->putVarInt($netId);
    ...
    public static function getItemStack(DataPacket $packet, \Closure $readExtraCrapInTheMiddle, int $protocol) : Item{
        ...
        $null = null;
        [$id, $meta] = XMultiVersionItemTranslator::getInstance()->fromNetworkId($netId, $netData, $null, $protocol);
        ...
        return (static function() use ($protocol, $extraData, $netId, $id, $meta, $cnt) : Item{
            ...
            return ItemTypeIds::get($id, $meta, $cnt, $nbt);
        })();
    }
